...
function sandbagRun(scene, colliders, { x, z, length, height, along, ground }) {
  const unit = 1.6;
...
function watchtower(scene, colliders, { x, z, ground, color }) {
  const HEIGHT = 5.4;
  const LEG = 0.35;
  const PLATFORM = 3.2;
  ...
  addBox(scene, colliders, { x, y: ground + HEIGHT + 0.25, z: z - edge, w: PLATFORM, h: rail, d: 0.16, color: WOOD });
  addBox(scene, colliders, { x: x - edge, y: ground + HEIGHT + 0.25, z, w: 0.16, h: rail, d: PLATFORM, color: WOOD });
  addBox(scene, colliders, { x: x + edge, y: ground + HEIGHT + 0.25, z, w: 0.16, h: rail, d: PLATFORM, color: WOOD });
...
function bunker(scene, colliders, { x, z, ground }) {
  const W = 7;
  const D = 5;
  const H = 2.6;
  const DOOR = 1.4;

  addBox(scene, colliders, { x, y: ground, z: z - D / 2, w: W, h: H, d: 0.5, color: CONCRETE });
  addBox(scene, colliders, { x: x - W / 2, y: ground, z, w: 0.5, h: H, d: D, color: CONCRETE });
  addBox(scene, colliders, { x: x + W / 2, y: ground, z, w: 0.5, h: H, d: D, color: CONCRETE });
  ...
  addBox(scene, colliders, { x, y: ground + H, z, w: W + 0.4, h: 0.4, d: D + 0.4, color: CONCRETE });
}

/** Barraca de campanha: pirâmide de lona sobre um estrado. */
function tent(scene, colliders, { x, z, ground }) {
  addBox(scene, colliders, { x, y: ground, z, w: 3.4, h: 0.9, d: 3.4, color: CANVAS_TENT });
...
function barrel(scene, colliders, { x, z, ground, color }) {
 ...
 */
export function addBase(scene, colliders, { name, x = 0, z, ground, facing, color }) {
  const front = facing;

  // perímetro: dois lados inteiros, e a frente partida pelo portão
  sandbagRun(scene, colliders, { x, z: z - HALF, length: HALF * 2, height: WALL_HEIGHT, along: 'x', ground });
  sandbagRun(scene, colliders, { x: x - HALF, z, length: HALF * 2, height: WALL_HEIGHT, along: 'z', ground });
  sandbagRun(scene, colliders, { x: x + HALF, z, length: HALF * 2, height: WALL_HEIGHT, along: 'z', ground });

  const wing = HALF - GATE_HALF;
  for (const sx of [-1, 1]) {
    sandbagRun(scene, colliders, {
      x: x + sx * (GATE_HALF + wing / 2), z: z + HALF,
  ...
  watchtower(scene, colliders, { x: x - 11, z: z - 9 * front, ground, color });
  bunker(scene, colliders, { x: x + 8, z: z - 6 * front, ground });
  tent(scene, colliders, { x: x - 4, z: z + 8 * front, ground });
  tent(scene, colliders, { x: x + 3, z: z + 10 * front, ground });
  ...
  for (let i = 0; i < 4; i++) {
    barrel(scene, colliders, {
      x: x + 13 + (i % 2) * 0.8, z: z + (5 + Math.floor(i / 2) * 0.9) * front,
